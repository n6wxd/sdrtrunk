    ...
    IReusableBufferProvider, Listener<ReusableComplexBuffer>
{
     ...
     */
    public AMDemodulatorModule(double channelBandwidth, double outputSampleRate, double squelchLevel, boolean squelchMode)
    {
    ...
    public void receive(ReusableComplexBuffer basebandBuffer)
    {
		...
		if (display_enable == 0) 
		{
			mLog.info("             rssi:" + mAGCSquelch.getRssi());
			mLog.info("Squelch Threshold:" + mAGCSquelch.getSquelchThreshold());
			mLog.info("     Squelch Mode:" + mAGCSquelch.getSquelchMode());
			mLog.info("                  " );
		}
    }
